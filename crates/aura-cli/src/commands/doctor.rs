...
pub fn execute_doctor() {
    ...
    println!("Cores (Phys/Log) : {} / {}", hw.cpu.physical_cores, hw.cpu.logical_cores);
    println!("Base Clock       : {:.2} GHz", hw.cpu.base_clock_ghz);
    
    let simd_str: Vec<String> = hw.cpu.simd_features.iter().map(|s| format!("{:?}", s)).collect();
    println!("SIMD Extensions  : {}", simd_str.join(", "));

    println!("\n=== MEMORY TOPOLOGY ===");
    println!("Total Physical RAM : {:.2} GB", hw.memory.total_ram_bytes as f64 / 1e9);
    println!("Available RAM      : {:.2} GB", hw.memory.available_ram_bytes as f64 / 1e9);
    println!("Total Swap Space   : {:.2} GB", hw.memory.total_swap_bytes as f64 / 1e9);
